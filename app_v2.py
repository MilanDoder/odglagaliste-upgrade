...
def _ucitaj_podatke(gost: bool = False):
    ...
    # ekonomski koeficijenti — default iz fajla parametara (4. i 5. broj)
    # ili MATLAB vrijednosti; podesivo u sidebaru za bilo koju lokaciju
    with st.sidebar.expander("Ekonomski koeficijenti"):
        par.cijena_transporta = st.number_input(
            "Transport (valuta po m³ po km)", min_value=0.0,
            value=float(par.cijena_transporta
                        if par.cijena_transporta is not None else 0.8),
            step=0.05, format="%.3f")
        par.cijena_dizanja = st.number_input(
            "Dizanje (valuta po m³ po m visine)", min_value=0.0,
            value=float(par.cijena_dizanja
                        if par.cijena_dizanja is not None else 0.024),
...
_FALLBACK_BOJE = ["teal", "gold", "crimson", "royalblue", "darkorange",
                  "mediumseagreen", "slateblue", "salmon"]
...
def _boja_zone(naziv: str) -> str:
    for prefiks, boja in BOJE_ZONA:
        if naziv.startswith(prefiks):
            return boja
    # nepoznat naziv (klijentski šifarnik) → stabilna boja iz palete
    return _FALLBACK_BOJE[hash(naziv.split(".")[0]) % len(_FALLBACK_BOJE)]
...
if stranica_profila(autentikator, _korisnik):
    st.stop()
...
sidebar_footer(autentikator, _korisnik)
