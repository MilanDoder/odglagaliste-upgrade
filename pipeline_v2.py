...
def ekonomska_cijena_v2(rez, dobre_zone: list,
    ...
    if precizno:
        ...
        cena_mapa = {z.naziv: z.cena for z in dobre_zone}
        ukupno = sum(cijena_zone(naziv, p, cena_mapa.get(naziv))
                     for naziv, p in po_zonama.items())
    ...
    for zona in dobre_zone:
        ...
        if np.any(MplPath(zp).contains_points(tacke)):
            ukupno += cijena_zone(zona.naziv, zona.povrsina, zona.cena)
...
class KontekstV2:
    ...
    # ekonomski koeficijenti funkcije cilja (default = MATLAB original):
    #   c1 = distanca_km · cijena_transporta · V     [0.8 valuta/(m³·km)]
    #   c2 = V · max(wz − mnv, 0) · cijena_dizanja   [0.024 = 1.6·1.2/(0.08·1000)]
...
def _evaluiraj(wz: float, k: float, wx: float, wy: float,
    ...
    def _ocijeni(rez_dio, V):
        ...
        # i koeficijentima iz konteksta (default = MATLAB vrijednosti)
        c1 = (distanca / 1000.0) * ctx.cijena_transporta * V
        # max(0, ...): kota vrha ispod mnv NE smije davati negativan (nagradni)
        # trošak dizanja — bez ovoga GA 'bježi' u kop da naplati kopanje
        c2 = V * ((max(wz - ctx.mnv, 0.0) / 0.08 * 1.6) / 1000.0) \
             * (ctx.cijena_dizanja / (1.6 / (0.08 * 1000.0)))
...
def proracun_tacke(
    ...
    c1, c2, c3 = racunaj_troskove(rez.zapremina, dist, wz, eko, mnv=ctx.mnv,
                                  cijena_transporta=ctx.cijena_transporta,
                                  cijena_dizanja=ctx.cijena_dizanja)
