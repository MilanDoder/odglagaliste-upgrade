...
class DodatniParametri:
    ...
    cijena_transporta: float = None    # valuta/(m³·km); default 0.8 (MATLAB) ako nije u fajlu
    cijena_dizanja: float = None       # valuta/(m³·m visine); default 0.024 (MATLAB) ako nije u fajlu
...
def _pocetak_zone(linije: list[str], i: int) -> bool:
    """Da li na liniji i počinje blok zone (5 linija)?

    Legacy (Buvač): naziv matchuje ^[ZK]-broj.
    Generički: bilo koji ne-numerički naziv iza kojeg slijede
    cijena (broj), površina (broj) i dvije linije sa ≥3 koordinate
    odvojene zarezima — pa klijent može koristiti svoja imena zona
    (npr. "PARCELA-12", "ZABRANJENA-3").
    """
    l = linije[i]
    if re.match(r'^[ZK]-\d', l):
        return True
    if not l or l.startswith(("%", "#")) or _je_broj(l) or "," in l:
        return False
    if i + 4 >= len(linije):
        return False
    if not (_je_broj(linije[i + 1]) and _je_broj(linije[i + 2])):
        return False
    x = np.fromstring(linije[i + 3], sep=",")
    y = np.fromstring(linije[i + 4], sep=",")
...
def ucitaj_ekonomske_zone(putanja: str | Path) -> tuple[list[EkonomskaZona], list[EkonomskaZona]]:
    ...
    while i < len(linije):
        ...
        if _pocetak_zone(linije, i):
            ...
            # Klasifikacija (Buvač konvencija + generička pravila):
            #   K-* i legacy Z-5  → zabranjene (loše)
            #   naziv sadrži "ZABRAN" (npr. "ZABRANJENA-3") → zabranjena
            #   sve ostalo → dozvoljene (dobre) sa cijenom zemljišta
            gornji = naziv.upper()
            if (gornji.startswith("K") or gornji.startswith("Z-5")
                    or "ZABRAN" in gornji):
                lose.append(zona)
            else:
                dobre.append(zona)
...
def ucitaj_dodatne_parametre(putanja: str | Path) -> DodatniParametri:
    ...
    params = DodatniParametri(
        ...
        cijena_transporta=brojevi[3] if len(brojevi) > 3 else None,
        cijena_dizanja=brojevi[4] if len(brojevi) > 4 else None,
    )
    eko = ""
    if params.cijena_transporta is not None:
        eko += f", transport={params.cijena_transporta}"
    if params.cijena_dizanja is not None:
        eko += f", dizanje={params.cijena_dizanja}"
    print(f"OK (mv={params.nadmorska_visina}, generacije={params.broj_generacija}, "
          f"dist={params.uslov_distance}{eko})")
    return params
